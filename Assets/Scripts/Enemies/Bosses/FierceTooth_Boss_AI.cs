...
public class FierceTooth_Boss_AI : MonoBehaviour
{
    [Header("Behavior Settings")]
    [SerializeField] private float walkSpeed = 2.5f;
    [SerializeField] private float nearDistance = 3f;
    [SerializeField] private float midDistance = 8f;
    [SerializeField] private float attackCooldown = 3f;

    [Header("Rush Settings")]
    [SerializeField] private float rushMaxSpeed = 18f;
    [SerializeField] private float rushDuration = 0.6f;
    [SerializeField] private float decelerateTime = 0.2f;

    [Header("Jump Settings")]
    [SerializeField] private float jumpHeight = 12f;
    [SerializeField] private float jumpHorizontalForce = 6f;
    ...
    private IEnumerator RecoilCoroutine()
    {
        ...
        animator.Play("FierceTooth_Idle");
    ...
    {
        isAttacking = true;
        animator.Play("FierceTooth_Run");
        AudioManagerController.Instance?.PlaySFX(AudioManagerController.Instance.bossDashSFX);

        float dir = player.position.x > transform.position.x ? 1 : -1;
        float timer = 0;

        while (timer < rushDuration - decelerateTime)
        {
            rb.linearVelocity = new Vector2(dir * rushMaxSpeed, rb.linearVelocity.y);
            timer += Time.deltaTime;
            yield return null;
        }

        float decelTimer = 0;
        while (decelTimer < decelerateTime)
        {
            float lerp = 1f - (decelTimer / decelerateTime);
            rb.linearVelocity = new Vector2(dir * rushMaxSpeed * lerp, rb.linearVelocity.y);
